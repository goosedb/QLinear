...
det (Matrix (n, _) matrix) = sum $ map (unsafeAlgebraicComplement matrix n) indices
  where
...
unsafeAlgebraicComplement matrix n (i, j) = aij * k * det (Matrix (n - 1, n - 1) minor)
  where
    (aij, minor) = cross matrix (i - 1, j - 1)
    k = fromIntegral $ (-1) ^ (j + i)

cross :: [[a]] -> (Int, Int) -> (a, [[a]])
cross matrix (i, j) = (line !! j, withoutColumn)
  where
